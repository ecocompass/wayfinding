          ...
          .map((trip, index) => (
            ...
            >
              <HStack space="lg" mt={"$4"}>
                <Text size="md" mb="$5">
                  {JSON.parse(trip.start_location).location_name +
                    ' -> ' +
                    JSON.parse(trip.end_location).location_name}{' '}
                </Text>
              </HStack>
              <HStack space="4xl" mt={"$4"}>
                <Text size="sm" mb="$5">
                  Time: {formatTime(trip.end_time, trip.start_time)}{' '}
                </Text>
                <Text size="sm" mb="$5">
                  Distance: {totalDistance(trip)} km{' '}
                </Text>
              </HStack>
            </Card>
          ))}
